...
```json
{
  ...
  header: string,            // 简短标签，最多12个字符
  opitons: [                 // 选项2~4个，超过4个选项，后续的不会显示
    {
      label: string,         // 选项显示内容
      description: string    // 选项描述，解释选项内容
    }
  ],
  multiSelecct?: boolean     // 是否允许多选，默认：false
}
```
...
```json
{
  "question": "格式化数据使用哪一个库？",
  "header": "Library",
  "opitons": [
    { "label": "data-fns", "description": "轻量的格式化库" },
    { "label": "mount.js", "description": "经典格式化库，但输出较长" }
  ],
  "multiSelecct": false
}
```
...
带变量的形式：通过{}表示一个变量，实际会显示变量的值

> 使用 AskUserQuestion 询问用户选择哪一个方案
